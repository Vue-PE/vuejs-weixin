...
<div class="page consumer">
    ...
    <div class="shop-title-container order">
        ...
        <h1>{{htmlInfo.project_info.item_name}}</h1>
    ...
    <form class="order-form select-working">
        ...
        <section v-show="serType == 1">
            房间号：
            <input type="text" style="width: 150px;" v-model="roomNo">
        </section>
...
export default{
    ...
    methods: {
        ...
        submit(){
            ...
            this.$http.post(API.userSubmitOrder, {
                sid: this.$route.params.storeid,
                pid: this.$route.params.pid,
                tid: this.$route.params.tid,
                rid: this.roomNo,
                ...
                if(data.data.errorInfo) return
                this.$store.state.hideTip()
                this.$store.state.showTip({type: 'loading', content: '提交成功' })
                setTimeout(()=>{
